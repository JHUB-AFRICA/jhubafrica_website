...
export const getEvents = async (): Promise<EventItem[]> => {
  const response = await api.get<{ data: EventItem[] }>("/api/v1/events");
  return response.data.data;
};

export const addEvent = async (event: Omit<EventItem, "id">): Promise<EventItem> => {
  const response = await api.post<{ data: EventItem }>("/api/v1/admin/events", event);
  return response.data.data;
};

export const updateEvent = async (event: EventItem): Promise<EventItem> => {
  const response = await api.patch<{ data: EventItem }>(`/api/v1/admin/events/${event.id}`, event);
  return response.data.data;
};
